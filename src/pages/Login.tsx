...
const Login = () => {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        toast({ title: "Tạo tài khoản thành công", description: "Bạn có thể bắt đầu phân tích ngay." });
      } else {
  ...
  return (
    <div className="container flex items-center justify-center py-16 md:py-24 min-h-[calc(100vh-4rem)]">
        ...
        <Card className="border-border/30 bg-card/78 p-8 shadow-elevated backdrop-blur-xl">
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">
              {mode === "signin"
                ? "Truy cập dashboard phân tích thị trường"
                : "Bắt đầu phân tích thị trường tuyển dụng theo tuần"}
            </p>
          </div>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <Button type="submit" className="w-full bg-gradient-data text-primary-foreground hover:opacity-90 transition-opacity" disabled={loading}>
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : mode === "signin" ? "Đăng nhập" : "Tạo tài khoản"}
            </Button>
          </form>
